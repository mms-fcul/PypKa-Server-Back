...
def insert_new_submission(conn, cur, cur_date, to_insert, pdbid, params, pdbfile, outputemail, error):
    ...
    input_id = insert_new_input(conn, cur, job_id, protein_id, pypka_set, pb_set, mc_set)
    results_id = insert_new_results(conn, cur, job_id, titration, pdb_out, error)
    res_id , pk_id = insert_new_residue_pk(conn, cur, protein_id, res_name, chain, res_number, pK, job_id)
...
def insert_new_protein(conn, cur, pdbid, pdbfile):
    ...
    """
    protein_id = executeSingleSQLstatement(conn, cur, sql_protein, commit=True, fetchall=True)[0][0]
    return protein_id
...
def insert_new_job(conn, cur, dat_time, outputemail):    
    ...
    RETURNING job_id;
    """
    job_id = executeSingleSQLstatement(conn, cur, sql_job, commit=True, fetchall= True)[0][0]
    return job_id
...
def insert_new_input(conn, cur, job_id, protein_id, pypka_set, pb_set, mc_set):
    ...
    RETURNING input_id;
    """
    input_id = executeSingleSQLstatement(conn, cur, sql_input, commit=True, fetchall= True)[0][0]
    return input_id
...
def insert_new_results(conn, cur, job_id, tit_curve, pdb_out, error):
    ...
    """
    results_id = executeSingleSQLstatement(conn, cur, sql_results, commit=True, fetchall= True)[0][0]
    return results_id

def insert_new_residue_pk(conn, cur, protein_id, res_name, chain, res_number,pK, job_id):
    ...
    for name,cha,number,pk in zip(res_name, chain, res_number, pK):
        ...
        RETURNING res_id;
        """
        res_id = executeSingleSQLstatement(conn, cur, sql_residue, commit=True, fetchall= True)[0][0]
        sql_pk= f"""
        ...
        pk_id = executeSingleSQLstatement(conn, cur, sql_pk, commit=True, fetchall= True)[0][0]
    return res_id, pk_id
